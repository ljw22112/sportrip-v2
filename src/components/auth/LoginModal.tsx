        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {step===1&&(
            <div>
              ...
              <p className="text-sm text-[#717171] text-center mb-6">닉네임을 입력하면 AI가 맞춤 관광코스를 추천해드립니다</p>
              ...
              <button onClick={()=>setStep(2)} disabled={nickname.length<2}
                className={cn('w-full mt-4 py-3.5 rounded-xl font-bold text-sm transition-colors',nickname.length>=2?'bg-[#FF5722] text-white hover:bg-[#FF7043]':'bg-[#EBEBEB] text-[#AAAAAA] cursor-not-allowed')}>
                다음 →
              </button>
            </div>
          )}
          {step===2&&(
            <div>
              ...
              <p className="text-sm text-[#717171] mb-5">Q-Learning AI가 선택에 맞는 코스를 추천합니다</p>
              <div className="space-y-3">
                {COMPANIONS.map(c=>(
                  <button key={c.id} onClick={()=>setCompanion(c.id)}
                    className={cn('w-full flex items-center gap-4 p-4 rounded-2xl border-2 transition-all text-left',companion===c.id?'border-[#222222] bg-[#F7F7F7]':'border-[#EBEBEB] hover:border-[#AAAAAA]')}>
                    <span className="text-3xl">{c.icon}</span>
                    <div className="flex-1"><div className="font-semibold text-sm text-[#222222]">{c.label}</div><div className="text-xs text-[#717171]">{c.desc}</div></div>
          ...
          {step===3&&(
            <div>
              ...
              <p className="text-sm text-[#717171] text-center mb-4">필수 동의 후 서비스를 이용할 수 있습니다</p>
              <button onClick={()=>{setAgreeTerms(true);setAgreePrivacy(true);}}
                className={cn('w-full flex items-center gap-3 p-4 rounded-2xl border-2 mb-3 transition-all',agreeTerms&&agreePrivacy?'border-[#222222] bg-[#F7F7F7]':'border-[#EBEBEB]')}>
              ...
              {[{flag:agreeTerms,setFlag:setAgreeTerms,show:showTerms,setShow:setShowTerms,label:'[필수] 서비스 이용약관',text:TERMS},
                ...
                <div key={i} className="border border-[#EBEBEB] rounded-2xl overflow-hidden mb-3">
                  <div className="flex items-center gap-3 p-4">
                    ...
                    <button onClick={()=>item.setShow((v:boolean)=>!v)} className="text-[#717171]">{item.show?<ChevronUp className="w-4 h-4"/>:<ChevronDown className="w-4 h-4"/>}</button>
                  </div>
                  {item.show&&<div className="px-4 pb-4 border-t border-[#EBEBEB]"><pre className="text-xs text-[#717171] whitespace-pre-wrap font-sans leading-relaxed bg-[#F7F7F7] p-3 rounded-xl max-h-28 overflow-y-auto">{item.text}</pre></div>}
                </div>
              ))}
              ...
              <button onClick={done} disabled={!agreeTerms||!agreePrivacy}
                className={cn('w-full py-3.5 rounded-xl font-bold text-sm transition-colors',agreeTerms&&agreePrivacy?'bg-[#FF5722] text-white hover:bg-[#FF7043]':'bg-[#EBEBEB] text-[#AAAAAA] cursor-not-allowed')}>
                {nickname}님, 시작하기 🚀
              </button>
            </div>
          )}
