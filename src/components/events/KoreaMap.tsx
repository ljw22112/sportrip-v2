...
export function KoreaMap({ events, className }: Props) {
  ...
  const FILTER_SPORTS = SPORTS_15.slice(0,9);

  return (
    <div className="relative w-full h-full flex flex-col">
      {/* 종목 필터 칩 */}
      <div className="absolute top-2.5 left-2.5 right-2.5 z-10 flex gap-1.5 overflow-x-auto pb-1"
        style={{scrollbarWidth:'none'}}>
        {FILTER_SPORTS.map(sp=>(
          <button key={sp.key} onClick={()=>setActiveSport(sp.key)}
            className={`flex-shrink-0 flex items-center gap-1 px-2.5 py-1.5 rounded-full text-[12px] font-bold border-2 transition-all shadow-sm
              ${activeSport===sp.key ? 'text-white border-transparent shadow-md' : 'bg-white/95 text-[#222] border-white hover:border-[#DDD]'}`}
      ...
      {/* 지도 SVG */}
      <svg viewBox="0 0 520 540" className={className} style={{width:'100%',height:'100%',flex:1}}>
        <defs>
          <radialGradient id="seaGrad" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#C8DCE8"/>
            <stop offset="100%" stopColor="#B0CCE0"/>
          </radialGradient>
        ...
        {/* 바다 배경 */}
        <rect width="520" height="540" fill="url(#seaGrad)" rx="16"/>

        {/* ── 한반도 본토 ── */}
        {/* 정밀 남한 윤곽 — 주요 반도·만입 포함 */}
        <path filter="url(#landShadow)" fill="#B8CDB5" stroke="#7A9E7E" strokeWidth="1.2"
          d="
            M 158 72
            L 178 64 L 210 60 L 248 60 L 282 65 L 315 72
            C 332 82, 345 98, 350 118
            L 354 145 L 356 170 L 357 195
            L 358 220 L 360 248
            ...
            L 122 135 L 128 122
            C 135 108, 144 95, 155 84
            Z
          "/>

        {/* 서쪽 주요 섬들 (황해) — 강화도, 진도 등 */}
        ...
        {/* 남쪽 섬들 — 거제, 남해 등 */}
        <path fill="#B8CDB5" stroke="#7A9E7E" strokeWidth="0.8"
          d="M 300 375 L 312 370 L 320 378 L 308 382 Z"/>
        ...
        {/* ── 제주도 ── */}
        <ellipse cx="218" cy="468" rx="52" ry="22" fill="#B8CDB5" stroke="#7A9E7E" strokeWidth="1.2" filter="url(#landShadow)"/>
        <text x="218" y="471" textAnchor="middle" fontSize="10" fill="#4A6741" fontWeight="700">제주</text>

        {/* ── 울릉도 ── */}
        <circle cx="432" cy="185" r="11" fill="#B8CDB5" stroke="#7A9E7E" strokeWidth="1.2" filter="url(#landShadow)"/>
        ...
        {/* 동해 / 황해 라벨 */}
        <text x="450" y="155" textAnchor="middle" fontSize="11" fill="#6892A8" fontWeight="600" opacity="0.7">동해</text>
        ...
        {Object.entries(clusters).map(([region, evs])=>{
          ...
          const colors = ['#E4572E','#2E86C1','#27AE60','#8E44AD','#D4AC0D'];
          const color = colors[Object.keys(REGION_POS).indexOf(region) % colors.length];
      ...
      {/* 팝업 */}
      {popup && (
        <div className="absolute inset-x-2 bottom-2 bg-white/97 border-2 border-[#DDD] rounded-2xl shadow-xl z-20 max-h-[52%] overflow-hidden flex flex-col">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#EEE] flex-shrink-0">
            ...
            <button onClick={()=>setPopup(null)} className="text-[#717171] hover:text-[#222] text-2xl font-light w-8 h-8 flex items-center justify-center leading-none">×</button>
          </div>
      ...
      {!popup && (
        <div className="absolute left-2.5 bottom-2.5 bg-white/90 backdrop-blur-sm rounded-full text-[11px] text-[#717171] px-3 py-1.5 font-medium">
