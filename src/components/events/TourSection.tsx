  ...
  return (
    <div className="mb-8">
      <div className="flex items-center gap-2 mb-4">
        <h3 className="text-[17px] font-bold text-[#222]">{icon} {title}</h3>
        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
          loading ? 'bg-gray-100 text-gray-400' :
          showApi ? 'bg-[#E7F1EC] text-[#0B5C43]' : 'bg-gray-100 text-gray-500'
        }`}>
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[1,2,3,4].map(i=>(
            <div key={i} className="bg-[#F7F7F6] border border-[#EBEBEB] rounded-xl p-4 animate-pulse">
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {displayItems.map((item, i) => {
              ...
              const card = (
                <div key={i}
                  className={`bg-[#F7F7F6] border border-[#EBEBEB] rounded-xl overflow-hidden transition-all
                    ${url ? 'cursor-pointer hover:border-[#0B5C43] hover:shadow-sm' : ''}`}
                  onClick={() => url && window.open(url, '_blank')}>
                  {img && (
                    <img src={img} alt={name} className="w-full h-24 object-cover"/>
                  )}
                  <div className="p-3">
                    <b className="block text-[13px] font-bold text-[#222] mb-1 line-clamp-2 leading-tight">{name}</b>
                    {addr && (
                      <div className="flex items-start gap-1 text-[11px] text-[#717171]">
                        <MapPin className="w-3 h-3 mt-0.5 flex-shrink-0"/>
                        <span className="line-clamp-1">{addr}</span>
                      </div>
                    )}
                    {tel && (
                      <div className="flex items-center gap-1 text-[11px] text-[#0B5C43] mt-0.5">
                    ...
                    {url && (
                      <div className="flex items-center gap-1 text-[11px] text-[#0B5C43] mt-1.5 font-semibold">
          ...
          {hasMore && (
            <button onClick={() => setShowAll(v => !v)}
              className="mt-3 w-full flex items-center justify-center gap-1.5 py-2.5 border border-[#EBEBEB] rounded-xl text-[13px] font-semibold text-[#555] hover:border-[#0B5C43] hover:text-[#0B5C43] transition-all bg-white">
              {showAll
