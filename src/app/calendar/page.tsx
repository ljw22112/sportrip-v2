        ...
        <div className="flex gap-2 overflow-x-auto pb-3 mb-5 pt-1" style={{scrollbarWidth:'none'}}>
          {SPORTS_15.map(sp=>(
            <button key={sp.key} onClick={()=>{ setActiveSport(''); setTimeout(()=>setActiveSport(sp.label),0); }}
              className="flex items-center gap-2 px-3.5 py-2 rounded-full text-[13px] font-semibold flex-shrink-0 border-2 transition-all bg-white"
              style={activeSport===sp.label
                ? {borderColor:sp.color, color:sp.color, boxShadow:`0 0 0 1px ${sp.color}`}
