  ...
  return (
    <>
      ...
      <main>
        ...
        {/* ── 통계 바 ── */}
        <section className="bg-[bg-bg] border-b border-[#E8E8E4]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={s.label} className="text-center">
                  ...
                  <div className="text-[15px] font-bold text-ink mt-0.5">{s.label}</div>
        ...
        <section className="max-w-[1760px] mx-auto px-6 md:px-20 py-14">
          <div className="mb-10">
            ...
            <div className="w-12 h-1 bg-[bg-accent] mt-3 rounded-full"/>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURES.map(f=>(
              <div key={f.title} className="p-6 bg-white border border-border rounded-2xl hover:border-[bg-primary] hover:shadow-sm transition-all">
                <IconTile name={f.icon} tone={f.tone} size={24}/>
                <h3 className="text-[17px] font-bold text-ink mt-4 mb-2">{f.title}</h3>
        ...
        <section style={{background:'bg-bg-warm'}}>
          <div className="max-w-[1760px] mx-auto px-6 md:px-20 py-14">
            <div className="mb-10">
              <p className="text-[13px] font-bold text-[bg-primary] uppercase tracking-widest mb-2">DATA SOURCES</p>
              ...
              {DATA_SOURCES.map(d=>(
                <div key={d.name} className="bg-white border border-[#DDDAD5] rounded-2xl p-6">
                  <div className="flex items-start justify-between mb-5">
                    ...
                    <span className="text-[11px] font-bold bg-[bg-primary-tint] text-[bg-primary] px-3 py-1 rounded-full">{d.badge}</span>
                  </div>
                  <h3 className="text-[19px] font-bold text-ink mb-0.5">{d.name}</h3>
                  <p className="text-[13px] text-[bg-primary] font-semibold mb-3">{d.sub}</p>
                  <p className="text-[14px] text-[#555] leading-relaxed mb-4">{d.desc}</p>
                  <a href={d.url} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-[bg-primary] hover:underline">
                    공식 사이트 방문
            ...
            <p className="text-[12px] text-faint mt-6 text-center">출처: ⓒ한국관광공사 | 공공데이터포털 전국대회정보 표준데이터</p>
          </div>
        </section>
        ...
        <section className="max-w-[1760px] mx-auto px-6 md:px-20 py-14">
          <div className="mb-10">
            <p className="text-[13px] font-bold text-[bg-primary] uppercase tracking-widest mb-2">TECH STACK</p>
            <h2 className="text-[28px] md:text-[36px] font-extrabold tracking-tight">기술 스택</h2>
            <div className="w-12 h-1 bg-[bg-accent] mt-3 rounded-full"/>
          </div>
          <div className="flex flex-wrap gap-3">
            {TECH_STACK.map(t=>(
              <div key={t.name} className="flex items-center gap-3 px-5 py-3.5 bg-white border border-border rounded-2xl hover:border-[bg-primary] transition-colors">
                <SportripIcon name={t.icon} size={22} className="text-[bg-primary]"/>
                <div>
        ...
        <section style={{background:'bg-bg-warm'}}>
          <div className="max-w-[1760px] mx-auto px-6 md:px-20 py-14">
            <div className="mb-10">
              <p className="text-[13px] font-bold text-[bg-primary] uppercase tracking-widest mb-2">ROADMAP</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
              {ROADMAP.map(r=>(
                <div key={r.phase} className={`p-5 rounded-2xl border-2 ${
                  r.status==='done'    ? 'bg-[bg-primary-tint] border-[bg-primary]' :
                  r.status==='current' ? 'bg-white border-[bg-accent] shadow-md' :
                                         'bg-white border-[#DDDAD5]'}`}>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[11px] font-bold text-muted">{r.phase}</span>
                    <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${
                      r.status==='done'    ? 'bg-[bg-primary] text-white' :
                      r.status==='current' ? 'bg-[bg-accent] text-[#2A3308]' :
                                             'bg-[#EBEBEB] text-muted'}`}>
                      {r.status==='done' ? '완료' : r.status==='current' ? '진행 중' : '예정'}
                    </span>
                  </div>
                  <h3 className="text-[15px] font-bold text-ink mb-3">{r.title}</h3>
                  <ul className="space-y-1.5">
                    {r.items.map(item=>(
                      <li key={item} className="flex items-center gap-2 text-[13px] text-[#555]">
                        <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                          r.status==='done' ? 'bg-[bg-primary]' :
        ...
        <section className="max-w-[1760px] mx-auto px-6 md:px-20 py-14">
          <div className="mb-10">
            <p className="text-[13px] font-bold text-[bg-primary] uppercase tracking-widest mb-2">BUSINESS MODEL</p>
            <h2 className="text-[28px] md:text-[36px] font-extrabold tracking-tight">비즈니스 모델</h2>
            <div className="w-12 h-1 bg-[bg-accent] mt-3 rounded-full"/>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {BUSINESS.map(b=>(
              <div key={b.title} className="p-6 bg-white border border-border rounded-2xl">
                <div className="flex items-start justify-between mb-4">
                  <IconTile name={b.icon} tone={b.tone} size={24}/>
                  <span className="text-[11px] font-bold bg-[bg-bg] text-muted px-2.5 py-1 rounded-full">{b.badge}</span>
                </div>
                <h3 className="text-[16px] font-bold text-ink mb-2">{b.title}</h3>
                <p className="text-[14px] text-muted leading-relaxed">{b.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section style={{background:'bg-primary'}}>
          <div className="max-w-[1760px] mx-auto px-6 md:px-20 py-16 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <h2 className="text-[26px] md:text-[32px] font-extrabold text-[#0F0F0F] tracking-tight">지금 바로 시작하세요</h2>
              <p className="text-[#333] mt-1">{EVENTS.length}개 이상의 스포츠 대회와 주변 여행 정보를 스포트립에서 확인하세요.</p>
            </div>
        ...
        <footer style={{background:'#0F0F0F',color:'rgba(255,255,255,0.75)'}} className="text-[13px]">
          <div className="max-w-[1760px] mx-auto px-5 md:px-10 py-8 flex flex-wrap gap-6 items-start">
            ...
            <div className="ml-auto text-right text-[12px]">
              ...
              예정 대회 {EVENTS.filter((e:any)=>e.status!=='done').length}건
            </div>
          </div>
