...
export default function HomePage() {
  ...
  return (
    <div className="max-w-[1120px] mx-auto px-5">
      ...
      {/* ── 종목 카테고리 칩 (에어비앤비 카테고리 바) ── */}
      <div className="flex gap-0 overflow-x-auto no-scrollbar border-b border-[#E5E5E5] mb-6 -mx-5 px-5">
          ...
          return (
            <Link key={label} href={s ? `/events?sport=${s}` : '/events'}
              ...
              className={`flex flex-col items-center gap-1.5 px-5 py-3 flex-shrink-0 border-b-2 transition-all text-xs font-medium whitespace-nowrap
                ${active ? 'border-[#1A1A1A] text-[#1A1A1A] opacity-100' : 'border-transparent text-[#717171] opacity-70 hover:opacity-100 hover:border-[#AAAAAA]'}`}>
            ...
            <div className="absolute top-full left-0 mt-1 bg-white border border-[#E5E5E5] rounded-xl shadow-lg w-44 py-1 z-50">
              {DATE_LIST.map(d => (
                <button key={d} onClick={() => { setDate(d); setDateOpen(false); }}
                  className={`w-full text-left px-4 py-2.5 text-sm hover:bg-[#F7F7F7] transition-colors ${date === d ? 'font-semibold text-[#1A3A2A]' : 'text-[#1A1A1A]'}`}>{d}</button>
              ))}
            </div>
          )}
        </div>
            ...
            <div className="absolute top-full left-0 mt-1 bg-white border border-[#E5E5E5] rounded-xl shadow-lg w-40 py-1 z-50 max-h-64 overflow-y-auto">
              {SPORTS_LIST.map(s => (
                <button key={s} onClick={() => { setSport(s); setSportOpen(false); }}
                  className={`w-full text-left px-4 py-2.5 text-sm hover:bg-[#F7F7F7] transition-colors ${sport === s ? 'font-semibold text-[#1A3A2A]' : 'text-[#1A1A1A]'}`}>{s}</button>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="mt-14 mb-16">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-[#1A1A1A]">지도로 한눈에 보기</h2>
