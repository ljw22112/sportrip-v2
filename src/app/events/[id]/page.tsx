      ...
      <main className="max-w-[1760px] mx-auto px-5 md:px-10 pb-32 md:pb-16">
        ...
        <div className="flex items-center gap-3 py-4">
          ...
          <p className="text-[14px] text-muted">
            <Link href="/events" className="text-[bg-primary] font-semibold hover:underline">대회 일정</Link>
            {' › '}{ev.region} · {ev.sport}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_340px] gap-8 md:gap-12 items-start">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              {isDone && <span className="bg-[#F7F7F6] text-faint font-semibold text-[15px] px-4 py-2 rounded-full">종료된 대회</span>}
            </div>
            ...
            {/* 핵심 정보 테이블 */}
            <div className="border-2 border-border rounded-2xl overflow-hidden mb-5">
              {[
                ['📅 대회 기간', `${fmtDate(ev.start)}${ev.end && ev.end !== ev.start ? ` ~ ${fmtDate(ev.end)}` : ''}`],
                ['🏃 종목', ev.sport + (ev.distances ? ` · ${ev.distances}` : '')],
                ['👥 참가 규모', ev.participants],
              ].map(([label, val])=>(
                <div key={label} className="flex items-start gap-4 px-5 py-4 border-b border-border">
                  <span className="text-[15px] text-muted font-semibold w-32 flex-shrink-0">{label}</span>
                  <span className="text-[16px] font-medium text-ink flex-1">{val}</span>
                </div>
              ))}
              {/* 공식 사이트 */}
              ...
              <div className="flex items-center gap-4 px-5 py-3 bg-[#F7F7F6]">
                ...
                <span className="text-[11px] text-faint ml-auto">매일 자동 업데이트</span>
              </div>
            </div>

            {/* ── 개최 장소 + 길찾기 ── */}
            <div className="border-2 border-border rounded-2xl overflow-hidden mb-8">
              <div className="px-5 py-4 border-b border-border bg-[#F7F7F6]">
                <h3 className="font-bold text-[16px] text-ink">📍 개최 장소</h3>
              </div>
              <div className="px-5 py-4 border-b border-border">
                <div className="font-bold text-[17px] text-ink mb-1">{ev.venue}</div>
                <div className="text-[15px] text-muted">{ev.address} · {ev.region}</div>
                
              </div>
              {/* 지도 미리보기 + 길찾기 버튼 */}
              <div className="px-5 py-4">
                <p className="text-[13px] text-muted mb-3">외부 지도 앱으로 길찾기</p>
                <div className="flex flex-col sm:flex-row gap-2.5">
                  <a href={`https://map.naver.com/v5/directions/-/-/${encodeURIComponent(ev.venue)},${ev.lng},${ev.lat}/transit`}
                    target="_blank" rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-[15px] border-2 border-[#E0E0E0] text-[#333] hover:border-[bg-primary] hover:text-[bg-primary] transition-all bg-white">
            ...
            <section>
              ...
              <TourSection title="무장애 여행 (장애인·고령자·영유아)" icon="♿" sampleItems={tour.attraction} lat={ev.lat} lng={ev.lng} contentTypeId="15" barrierFree={true}/>
            ...
            <p className="text-[12px] text-faint mt-2 mb-8">
...
/>
              ...
              <div className="p-5">
                ...
                {ev.url ? (
                  ...
                  <div className="block w-full text-center py-4 rounded-xl text-[15px] text-faint bg-[#F7F7F6]">
                    공식 사이트 준비 중
                  </div>
                )}
